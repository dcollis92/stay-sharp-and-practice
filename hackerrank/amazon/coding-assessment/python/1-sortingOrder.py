...
def sortOrders(orderList):
  ...
  for i in orderList:
      temp = list(i.split())
      if temp[1].isnumeric():
          nonPrime.append(temp)
      else:
          prime.append(temp)

  # separate keys from the strings of prime orders
  for i in range(len(prime)):
      tempStr = ' '.join(prime[i][1:])
      tempKey = prime[i][0]
      prime[i] = [tempStr, tempKey]

  # sort in lexicographical order, python sorts by the first index by default
  prime.sort()

  # reformatting arrays for output
  for i in range(len(prime)):
